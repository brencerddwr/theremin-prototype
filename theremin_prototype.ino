...
#include "NewPing.h"	//ping library that handles multiple sensors and has more control
#include "toneAC.h"		//tone library that provides software volume control and louder output.
...
void oneSensorCycle() { // Sensor ping cycle complete, do something with the results.
  ...
  {
    tone_freq = map(pingtime[0], 1030, MAX_TIME, freq_top, freq_bottom); //MIN_TIME replaced 1030 (lowest reading seen on serial output)
  }
  ...
  {
    tone_volume = map(pingtime[1], 1030, MAX_TIME, 75, 1); //MIN_TIME replaced 1030 (lowest reading seen on serial output)
